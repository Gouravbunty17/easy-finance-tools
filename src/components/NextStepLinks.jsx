...
export default function NextStepLinks({ title = 'What to do next', intro, links = [] }) {
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-700 dark:bg-slate-900/60">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        {links.map((item) => (
          <Link key={item.href} to={item.href} className="rounded-2xl border border-slate-200 bg-white p-5 transition hover:border-secondary hover:shadow-sm dark:border-slate-700 dark:bg-gray-900">
