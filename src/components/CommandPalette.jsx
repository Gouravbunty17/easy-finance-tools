...
const items = [
  ...
  { icon: "FH", title: "FHSA Planner", category: "Savings", link: "/tools/fhsa-calculator" },
  ...
  { icon: "DV", title: "ETF Income Simulator", category: "Investing", link: "/tools/dividend-calculator" },
...
export default function CommandPalette({ open = false, onClose }) {
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center px-4 pt-24" onClick={close}>
      ...
      >
        <div className="flex items-center gap-3 border-b border-gray-100 px-4 py-3 dark:border-gray-800">
          <span className="text-sm font-semibold uppercase tracking-[0.18em] text-gray-500">Search</span>
          <input
            ref={inputRef}
            type="text"
            ...
            onKeyDown={onKeyDown}
            className="flex-1 bg-transparent text-sm text-gray-800 outline-none placeholder-gray-500 dark:text-white"
          />
          <kbd className="hidden rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-300 sm:block">Esc</kbd>
        </div>

        <ul className="max-h-80 overflow-y-auto py-2">
          {filtered.length === 0 ? (
            <li className="py-8 text-center text-sm text-gray-500">No results</li>
          ) : (
              ...
              <li key={item.link}>
                <button
                  ...
                  className={`flex w-full items-center gap-3 px-4 py-2.5 text-left transition-colors ${
                    index === selected ? "bg-primary/10 dark:bg-primary/20" : "hover:bg-gray-50 dark:hover:bg-gray-800"
                  }`}
                >
                  ...
                  <span className="shrink-0 text-xs text-gray-500">{item.category}</span>
                </button>
              </li>
            ))
          )}
        ...
        <div className="flex gap-4 border-t border-gray-100 px-4 py-2 text-xs text-gray-500 dark:border-gray-800 dark:text-gray-300">
          <span>
            <kbd className="rounded bg-gray-100 px-1 text-gray-600 dark:bg-gray-800 dark:text-gray-200">Up/Down</kbd> navigate
          </span>
          <span>
            <kbd className="rounded bg-gray-100 px-1 text-gray-600 dark:bg-gray-800 dark:text-gray-200">Enter</kbd> open
          </span>
          <span>
            <kbd className="rounded bg-gray-100 px-1 text-gray-600 dark:bg-gray-800 dark:text-gray-200">Esc</kbd> close
          </span>
        </div>
      </div>
    </div>
