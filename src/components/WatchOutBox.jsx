    ...
    <section className={`rounded-3xl border border-amber-200 bg-amber-50 p-6 text-amber-950 dark:border-amber-900/60 dark:bg-amber-950/25 dark:text-amber-100 ${className}`}>
      <div className="flex gap-3">
        ...
        <div>
          ...
          <h2 className="mt-2 text-2xl font-bold text-amber-950 dark:text-amber-100">{title}</h2>
      ...
      {items.length ? (
        <ul className="mt-5 grid gap-3 md:grid-cols-2">
          {items.map((item) => (
            <li key={item} className="rounded-2xl border border-amber-200/70 bg-white/75 p-4 text-sm leading-6 dark:border-amber-900/50 dark:bg-slate-900/55">
              {item}
            </li>
          ))}
        </ul>
